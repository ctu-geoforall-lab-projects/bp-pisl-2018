...
service. For \zk{WPS}, it is one of the three basic operations that are
...
outputs of the execute operation can be included in the response \zk{XML}
document or stored locally or on the web. In case of asynchronous
...
them to a\zk{WPS}server. It also provides functions to easily parse the
...
PostgreSQL and PostGIS use the client/server architecture. When a
